...
function stripAnswerKeysNonOwned(payload: any, ownerId: number | null | undefined) {
  const entries = Array.isArray(payload) ? payload : payload ? [payload] : [];
  for (const entry of entries) {
    if (entry.instructor?.id !== ownerId && Array.isArray(entry?.quizzes)) {
...
export default factories.createCoreController('api::course.course', ({ strapi }) => ({
  async find(ctx) {
    ...
    if (roleType === 'instructor') {
      stripAnswerKeysNonOwned(result.data, user.id);
    }
